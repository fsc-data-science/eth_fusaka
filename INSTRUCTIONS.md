...
1. Ensure all CSS is either:
   - Inlined in `<style>` tags, OR
   - Referenced via relative path (`styles.css`)
...
If provided, `rebuild-report.js` may assist with:
- File validation (ensuring all referenced files exist)
- Generating a checklist
- Simple string replacement/templating

However, full automation is not required—this INSTRUCTIONS file guides the assembly process.
